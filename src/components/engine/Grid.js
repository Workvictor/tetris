...
export class Grid {
  constructor({ width, height, cellSize, offset=0 }) {
    ...
    this.sprite = this.canvas.create.canvas({ width: this.width*cellSize, height: this.height*cellSize });
    this.linesDataSprites = Array(height).fill(null);
  }

  updateGameScore=(score)=>{
    const deltaScore = score - this.gameScore;
    const incrementStep = Math.floor(deltaScore/this.store.game.incrementCount);
    const scoreStack = Array(this.store.game.incrementCount).fill(incrementStep);
    const scoreStackSum = scoreStack.reduce((sum, val)=>sum+val, this.gameScore);
    scoreStackSum < score &&
    scoreStack.push( score - scoreStackSum );
  ...
  reset =()=>{
    this.data = Array(this.height).fill(Array(this.width).fill(0));
    this.sprite = this.canvas.create.canvas({ width: this.width*this.cellSize, height: this.height*this.cellSize });
  }

  get action(){
    const updateLinesCount =(linesCount=0, dropDelay=0)=> STORE.dispatch(ACTIONS.updateGame({
      linesCount, dropDelay
    }));
    return{
      updateLinesCount,
    }
  ...
  renderLinesData=()=>{
    ...
    this.linesDataSprites.forEach((elem, y) => {
      if(elem) buffer = this.canvas.compose.normal(buffer, elem, 0, y*this.cellSize);
    })
    this.sprite = buffer;
  }
  ...
  removeLine=(lineIndex, multiplierIndex)=>{
    ...
    multiplierIndex === 3 && this.store.audioController.playSoundByROLE('SFX_combo');
    const score = multiplierIndex > 0 ? lineScore*lineMultiplier*multiplierIndex : lineScore;
